...
int main(void) {
    ...
    //-------------------------------------------------------------------------------------
    for (;;) {
        // We need to run our task to process and deliver data for our IN and
        // OUT endpoints.
        HID_Task();
...
PORTD will toggle when printing is done.
    ...
    DDRB = 0xFF;  // uses PORTB. Micro can use either or, but both give us 2
                  // LEDs
...
void HID_Task(void) {
    ...
    if (Endpoint_IsOUTReceived()) {
        ...
        if (Endpoint_IsReadWriteAllowed()) {
            ...
            // However, since we're not doing anything with this data, we
            // abandon it.
        }
...
void GetNextReport(USB_JoystickReport_Input_t *const ReportData) {
    ...
    // 	if (pgm_read_byte(&(image_data[(xpos / 8) + (ypos * 40)])) & 1 << (xpos
    // % 8)) 		ReportData->Button |= SWITCH_A;
